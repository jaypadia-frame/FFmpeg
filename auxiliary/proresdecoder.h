...
/*!
 * Downscale modes. Modes smaller than full size may be used for fast,
 * draft-mode decoding and are only supported with the '2vuy' pixel format.
 */
typedef enum {
    kPRDownscale_1x1,                   ///!< Full size
    kPRDownscale_1x2,                   ///!< Full width x 1/2 height (single-field)
    kPRDownscale_2x2,                   ///!< 1/2 width x 1/2 height
    kPRDownscale_4x4,                   ///!< 1/4 width x 1/4 height
    kPRDownscale_8x8,                   ///!< 1/8 width x 1/8 height

    // Old enum names for backwards compatibility
    kPRFullSize     = kPRDownscale_1x1,
...
struct PRPixelBuffer {
    ...
    int             rowBytes;       //!< Number of bytes from first pixel of one line to first pixel of next line (must be at least PRBytesPerRowNeededInPixelBuffer() and a multiple of 16)
